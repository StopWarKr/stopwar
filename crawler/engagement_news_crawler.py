...
def get_news_data_from_current_page(source):
  ...
  for news in arr_news:
    ...
    src = news.find("img", attrs={"class" : "rISBZc zr758c M4dUYb"})
    #print(src)
    image_path = ""
    if(src != None):
      src = src['src']
...
# 크롤링이 돌아가는 메인 함수
def do_crawling_by_google(_driver, language, key_word, news_Xpath):
  _driver.get("https://google.com")

  search_bar = _driver.find_element_by_name('q')
  ...
  news_tag = _driver.find_element_by_xpath(news_Xpath)
  time.sleep(1)
  news_tag.click()
  time.sleep(3)
  # 뉴스 데이터 긁어온 후 josn파일 생성
  json_data = get_news_data_from_current_page(_driver.page_source) # 이미지의 src 정보를 온전히 가져오기 위해 page_source이용
...
def get_driver():
  ...
  #webdriver_options.add_argument("window-size=800x600") # headless 창의 크기를 몇으로 할건지 정의
  time.sleep(1)
...
time.sleep(2)
do_crawling_by_google(main_driver, "kr", "우크라이나 교전 상황", "//*[@id='hdtb-msb']/div[1]/div/div[4]/a")
time.sleep(5)
do_crawling_by_google(main_driver, "en", "Ukraine engagement", "//*[@id='hdtb-msb']/div[1]/div/div[3]/a")
main_driver.close()
print("Done")